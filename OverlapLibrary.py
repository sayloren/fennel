...
def intersect_overlaping_regions_by_element(df,overlap):
	...
	overlapTable = make_table_for_overlap_classification(pdOverlapBoundary)
	return pdOverlapBoundary, overlapTable

# Make a table for how many of each type of overlapping region
def make_table_for_overlap_classification(df):
...
def classify_overlap_regions_by_type_overlap(df):
	...
	if len(df[(df['startdifference'] >= 0) & (df['enddifference'] >= 0)]) != 0: # elements are completely inside the overlaps
		...
		completeelement = features[['chr','sBoundary','start','sEdge','sCenter','eCenter','eEdge','end','eBoundary','combineString','reverse_complement_dictionary','feature','id']]
		completeelement.columns = ['chr','sBoundary','start','sEdge','sCenter','eCenter','eEdge','end','eBoundary','combineString','reverse_complement_dictionary','feature','id']
	else:
	...
	if len(df[(df['startdifference'] <= -1) & (df['enddifference'] <= -1)]) != 0: # elements contain an overlaps
		...
		features['size'] = features['combineString'].str.len() # check length of string
		features['reverse_complement_dictionary'] = features.apply(lambda row: reverse_complement_dictionary(row['combineString']),axis=1)
		interiorelement = features[['chr','sBoundary','start','sEdge','sCenter','eCenter','eEdge','end','eBoundary','combineString','reverse_complement_dictionary','feature','id']]
		interiorelement.columns = ['chr','sBoundary','start','sEdge','sCenter','eCenter','eEdge','end','eBoundary','combineString','reverse_complement_dictionary','feature','id']
	else:
	...
	if len(df[(df['startdifference'] >= 0) & (df['enddifference'] <= -1)]) != 0: # elements overlap at the downstream boundary
		...
		features['reverse_complement_dictionary'] = features.apply(lambda row: reverse_complement_dictionary(row['combineString']),axis=1)
		features['size'] = features['combineString'].str.len() # check length of string
		endcrossboundary = features[['chr','sBoundary','newstart','sEdge','sCenter','eCenter','eEdge','newend','eBoundary','combineString','reverse_complement_dictionary','feature','id']]
		endcrossboundary.columns = ['chr','sBoundary','start','sEdge','sCenter','eCenter','eEdge','end','eBoundary','combineString','reverse_complement_dictionary','feature','id']
	else:
...
def main(rangeFeatures,fileName):
	...
	overlapFeatures, overlapTable = intersect_overlaping_regions_by_element(rangeFeatures,overFeature)
